...
resultadosCtrl.getResultados = async (req, res, next) => {
    ...
    const getData = async url => {
        try {
            ...
            console.log("RESP GET RESULTADOS: ", json.ip);
            var ip = json.ip;
...
resultadosCtrl.eliminarResultado = async (req, res, next) => {
    ...
    const getData = async url => {
        try {
            const response = await fetch(url);
            const json = await response.json();
            console.log("RESP ELIMINAR RESULTADOS: ", json.ip);
            var ip = json.ip;
            ...
            await Resultados.deleteOne({ "_id": id },{ "ip": ip});
...
resultadosCtrl.guardarResultadosPedestal = async (req, res, next) => {
    ...
    const getData = async url => {
        try {
            const response = await fetch(url);
            const json = await response.json();
            console.log("RESP GUARDAR RESULTADOS PEDESTAL: ", json.ip);
            var ip = json.ip;
            ...
            console.log("id: resultados eliminar: " + id);

            //.updateOne({_id : {$eq:1}} , {$set : {precio:15,cantidad:1} })
            //db.estudiantes.find( { edad : { $gt : 22 }, nombre : ‘Maria’ } )”


            //Se modifica datos de mongoDB
            const j = await Resultados.updateOne({ _id: id, ip: ip }, {
                $set: {
